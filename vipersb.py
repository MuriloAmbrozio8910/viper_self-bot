...
if os.name == "nt":
    ...
    os.system("title Viper SB")
if os.name == "posix":
    os.system("clear")

print(" ")
print(f"{printSpaces}Carregando Viper X SB...")
print(" ")
...
if not os.path.exists('logs/'): 
    os.makedirs('logs/')
    print(printSpaces+"Criando pasta de logs...")

open("logs/info.log", "w").write(" ")
print(printSpaces+"Resetando pasta de infos.")
open("logs/warning.log", "w").write(" ")
print(printSpaces+"Resetando pasta de Avisos.")
open("logs/error.log", "w").write(" ")
print(printSpaces+"Resetando pasta de Erros.")
open("logs/critical.log", "w").write(" ")
print(printSpaces+"Resetando pasta de Erros criticos.")
print(" ")
...
try: 
    ...
    def update_config():
        ...
        if ("riskmode" not in configFile):
            print(f"{printSpaces}Adicionando o modo de risco nas configuracoes.")
            configJson["riskmode"] = bool(False)

        if ("load_on_startup" not in configFile):
            print(f"{printSpaces}Adicionando carregamento ao inicializar.")
            configJson["load_on_startup"] = bool(False)

        if ("giveaway_join_delay" not in configFile):
            print(f"{printSpaces}Adicionando delay ao modo de Sorteios.")
            configJson["giveaway_join_delay"] = 15

        if ("giveaway_sniper_ui" not in configFile):
            print(printSpaces+"Adicionando modo sniper de sorteio.")
            configJson["giveaway_sniper_ui"] = False

        if ("snipers" not in configFile):
            configJson["snipers"] = {}
            print(printSpaces+"Adicionando sniper de nitro.")
            configJson["snipers"]["nitro"] = bool(False)
            print(printSpaces+"Adicionando privnote sniper na configuracao.")
            configJson["snipers"]["privnote"] = bool(False)
            print(printSpaces+"Adicionando giveaway sniper na configuracao.")
            configJson["snipers"]["giveaway"] = bool(False)

        if ("webhooks" not in configFile):
            configJson["webhooks"] = {}
            print(printSpaces+"Adicionando nitro webhook na configuracao.")
            configJson["webhooks"]["nitro"] = ""
            print(printSpaces+"Adicionando privnote webhook na configuracao.")
            configJson["webhooks"]["privnote"] = ""
            print(printSpaces+"Adicionando giveaway webhook na configuracao.")
            configJson["webhooks"]["giveaway"] = ""

        if ("motd" not in configFile):
            configJson["motd"] = {}
            configJson["motd"]["custom"] = bool(False)
            print(printSpaces+"Adicionando custom motd option na configuracao.")
        ...
        if ("viperping_detect" in configFile):
            configJson.pop("viperping_detect")
        ...
        if ("ignored_servers" not in configFile):
            ...
            configJson["ignored_servers"]["viperpings"] = []
            print(printSpaces+"Adding viperpings ignored servers to config.")
    ...
    if not os.path.isfile("data/icon.png"): open("data/icon.png", "wb").write(requests.get("https://raw.githubusercontent.com/viperSelfbot/Branding/main/viper.png", allow_redirects=True).content)
    # if not os.path.isfile('icon.ico'): open('icon.ico', 'wb').write(requests.get('https://viper.cool/favicon.ico', allow_redirects=True).content);  
    # if not os.path.isfile('sounds/connected.mp3'): open('sounds/connected.mp3', 'wb').write(requests.get('https://viper.cool/assets/sounds/connected.mp3', allow_redirects=True).content);  
    # if not os.path.isfile('sounds/error.mp3'): open('sounds/error.mp3', 'wb').write(requests.get('https://viper.cool/assets/sounds/error.mp3', allow_redirects=True).content);  
    # if not os.path.isfile('sounds/notification.mp3'): open('sounds/notification.mp3', 'wb').write(requests.get('https://viper.cool/assets/sounds/notification.mp3', allow_redirects=True).content);  
    # if not os.path.isfile('sounds/success.mp3'): open('sounds/success.mp3', 'wb').write(requests.get('https://viper.cool/assets/sounds/success.mp3', allow_redirects=True).content);  
    # if not os.path.isfile('sounds/giveaway-win.mp3'): open('sounds/giveaway-win.mp3', 'wb').write(requests.get('https://viper.cool/assets/sounds/giveaway-win.mp3', allow_redirects=True).content);  
    ...
    # open('data/icon.png', 'wb').write(requests.get('http://viper.cool/assets/icon.png', allow_redirects=True).content)
...
{
    "enabled": false,
    "client_id": 994444343232450611,
    "details": "Viper X SB The best",
    "state": "",
    "large_image_key": "",
    "large_image_text": ""
}
    ...
    if os.path.isfile("richpresence.json"):
        ...
        if jsonFile["client_id"] == 994444343232450611:
            jsonFile["client_id"] = 994444343232450611
        if jsonFile["details"] == "Viper X SB The best":
            jsonFile["details"] = ""
        if "small_image_key" not in jsonFile:
            jsonFile["small_image_key"] = ""
    ...
    if not os.path.isfile('themes/Viper.json'):
        f = open('themes/Viper.json', "w")
        f.write("""
{
    "embedtitle": "Viper X SB",
    "embedcolour": "#3B79FF",
    "consolecolour": "#3B79FF",
    "embedfooter": "Viper X Self Bot ",
    "embedfooterimage": "https://viper.cool/assets/icon.gif",
    "globalemoji": ":blue_heart:",
    "embedimage": "https://viper.cool/assets/icon.gif"
}
        """)
        f.close()
...
Never gonna tell a lie and hurt...""")
    ...
    if not os.path.isfile("scripts/consolecommand-example.py"):
        ...
        f.write("""
@viper.command(name="consolecommand", description="console command test", usage="consoletest", aliases=["consoleCommand-consoletest"])
...
""")
    ...
    if not os.path.isfile('scripts/example.py'):
        ...
        f.write('''
@viper.command(name="example", description="Example custom script.", usage="example")
async def example(viper):
    exampleEmbed = discord.Embed(
        ...
        ** **
        viper scripts are all created in python using discord.py so you can use any feature from discord.py.
        """,
    ...
    **\_\_deletetimeout\_\_** : Config delete timeout (seconds).
    ...
    await viper.send("Hello World!", embed=exampleEmbed)
    ''')
        f.close()
    ...
    __viperpingwebhook__ = CONFIG["webhooks"]["ghostping"]
    ...
    __viperpingdetect__ = CONFIG["detections"]["ghostping"]
    ...
    def print_important(message):
        print(f"{printSpaces}{fg.cGrey}[{getCurrentTime()}] {fg.cPurple}[IMPORTANTE] {fg.cWhite}{message}")    
    def print_info(message):
        print(f"{printSpaces}{fg.cGrey}[{getCurrentTime()}] {fg.cYellow}[INFORMACAO] {fg.cWhite}{message}")
    def print_cmd(command):
        print(f"{printSpaces}{fg.cGrey}[{getCurrentTime()}] {fg.consoleColour}[COMANDO] {fg.cWhite}{command}")
    def print_sharecmd(author, command):
        print(f"{printSpaces}{fg.cGrey}[{getCurrentTime()}] {fg.consoleColour}[COMANDOS DE OUTROS] {fg.cWhite}({author}) {command}")
    ...
    def is_me(m):
        return m.author == viper.user
    def restart_bot():
        python = sys.executable
        os.execl(python, python, * sys.argv)
    def close_bot():
        os.system("taskkill /IM viper.exe")
    def is_windows():
        return os.name == "nt"
    def is_linux():
    ...
    class Config():
        ...
        def changePrefix(newPrefix):
            ...
            viper.command_prefix = newPrefix
            cfg = Config.getConfig()
    ...
    if requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).status_code == 200:
        ...
        status = "online"
    viper = commands.Bot(command_prefix=__prefix__, self_bot=True, status=discord.Status.try_value(status))
    viper.remove_command('help')
    viper.launch_time = datetime.utcnow()
    ...
    cycleStatusText = ""
    cycleStatus = False
    discordServer = "discord.gg/viperx"
    uwuifyEnabled = False
    channelBlankChar = "᲼"
    ...
    async def on_connect():
        ...
        if is_windows():
            os.system("cls")
            os.system(f"title Viper SB [{version}] [{viper.user}]")
        ...
        threading.Thread(target=constant_input2, args=(viper,)).start()
        ...
        if consoleMode.lower() == "rise":
            ...
            print(fg.cGreen + "       ░                                                              ░ ".center(width))
            print("╭─━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━─╮")
            print(fg.cGrey + f"Connected: {viper.user} | Prefix: {viper.command_prefix} | Servers: {len(viper.guilds)}".center(width))
            print(fg.cBlue + "╰─━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━─╯")
            print("")
        ...
        if consoleMode.lower() == "nighty":
            ...
            print(f"{fg.cWhite}Account:   {viper.user} [{len(viper.guilds)} servers] [{len(get_friends(__token__))} friends]")
            print(f"{fg.cWhite}Prefix:    {viper.command_prefix}")
            ...
            # def print_cmd(command):
            #     print(f"{fg.cGrey}{getCurrentTime()} | {fg.consoleColour}[Command] {fg.cGrey} | {viper.command_prefix}{command}")
            # def print_sharecmd(author, command):
        ...
        if "beta" in version.lower():
            print_important("You're currently using a beta build of Viper X SB.")
            print_important("If you notice any bugs please report them to the developer.")
            print(" ")
        elif "dev" in version.lower():
            print_important("You're currently using a developer build of Viper X SB.")
            print_important("If you notice any bugs please report them to the developer.")
            print(" ")                
        ...
        print_info(f"Viper X SelfBot ja pode ser usado o Prefixo e: {viper.command_prefix}")
        send_notification("Viper SB", "Connectado com sucesso!", 10)  
        global __viperloaded__
        __viperloaded__ = True
    ...
    async def on_message_delete(message):
        if __viperloaded__:
            ...
            if Config.getConfig()["detections"]["ghostpings"]:
                if viper.user.mentioned_in(message):
                    if message.guild.id not in __ignoredservers__["ghostpings"]:
                        print_detect("viper Ping")
                        ...
                        if Config.getConfig()["sounds"]:   
                            if str(sounddevice.query_devices()) != "":          
                                ...
                                pygame.mixer.music.play(1)                        
                        send_notification("viper Ping", f"You were viper pinged in {message.guild} by {message.author}.", 10)
                        if __viperpingwebhook__ != "":
                            webhook = DiscordWebhook(url=__viperpingwebhook__)
                            embed = DiscordEmbed(title='viper Ping', color=__embedcolourraw__[1:], description=f"`{message.author}` viper pinged you in `{message.channel}` (`{message.guild}`)")
    ...
    async def on_message(message):
        if __viperloaded__:
            ...
            if message.author.id != viper.user.id:
                ...
                if Config.getConfig()["snipers"]["giveaway"]:
                    ...
                    if int(message.author.id) in giveawayBots and message.author.bot:
                        ...
                        if isGiveaway:
                            ...
                            #     if f"<@{viper.user.id}>" in message.content.lower():
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="customcommands", description="Display all custom commands.", usage="customcommands", aliases=["ccmds", "usercommands", "ucmds"])
...
{__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="search", description="Search for commands.", usage="search [term]", aliases=["find", "searchcommand"])
...
```""", delete_after=__deletetimeout__)
            ...
            text2 = ""
            searchedItems = 0
            for cmd in viper.commands:
                if command in cmd.name or command in cmd.description or command in cmd.aliases:
                    searchedItems += 1
                    text += f"`{viper.command_prefix}`**{cmd.usage}** » {cmd.description}\n"
                    text2 += f"{viper.command_prefix}{cmd.usage} » {cmd.description}\n"
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="help", description="The help command.", usage="help (command)", aliases=["cmds", "commands"])
    async def help(ctx, *, command = None):
        totalcmds = len(viper.commands)-len(scriptsList)
        if command is None:
            if __embedmode__:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
Precisa de argumentos `[]`, Argumentos opcionais `()`.

`{viper.command_prefix}`**text (page 1/2)** » Text commands.
`{viper.command_prefix}`**fun (page 1)** » Fun commands.
`{viper.command_prefix}`**image (page 1)** » Image commands.
`{viper.command_prefix}`**moderation (page 1)** » Moderation commands.
`{viper.command_prefix}`**info (page 1)** » Info commands.
`{viper.command_prefix}`**user (page 1)** » User commands.
`{viper.command_prefix}`**selfbot (page 1)** » Selfbot commands.
`{viper.command_prefix}`**webhook (page 1)** » Webhook commands.
`{viper.command_prefix}`**abuse (page 1)** » Abuse commands.
`{viper.command_prefix}`**themes (page 1)** » Theme commands.
`{viper.command_prefix}`**giveaway (page 1)** » Giveaway commands.
`{viper.command_prefix}`**nsfw (page 1)** » NSFW commands.
`{viper.command_prefix}`**proxy (page 1)** » Proxy commands.
`{viper.command_prefix}`**tools (page 1)** » Discord and other tools.
`{viper.command_prefix}`**customcommands** » Your custom commands.
`{viper.command_prefix}`**customscripts** » Your scripts.

`{viper.command_prefix}`**search [term]** » Search for a command.
`{viper.command_prefix}`**help (command)** » Help for a specific command.
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="themes", description="Theme related commands.", usage="themes")
    async def themes(ctx):
        ...
        if __embedmode__:
            ...
            embed.add_field(name="Commands", value=f"`{viper.command_prefix}`**newtheme [name]** » Create a new theme with the given name.\n`{viper.command_prefix}`**deltheme [name]** » Delete the named theme.\n`{viper.command_prefix}`**theme [theme]** » Change your current theme.\n`{viper.command_prefix}`**ctheme** » Community themes.", inline=False)
...
[ Commands ]
{viper.command_prefix}newtheme [name] » Create a new theme with the given name.
{viper.command_prefix}deltheme [name] » Delete the named theme.
{viper.command_prefix}theme [theme] » Change your current theme.
{viper.command_prefix}cthemes » Community themes.


# {__embedfooter__}```""", delete_after=__deletetimeout__)

    @viper.command(name="ctheme", description="Community themes.", usage="ctheme", aliases=["communitythemes", "cloudthemes", "cthemes"])
    async def ctheme(ctx, *, dl = None):
        if dl is None:
            url = "https://raw.githubusercontent.com/ySeuamorkkk/vipersb/main/temas.txt"
            themes = requests.get(url).text.split("\n")

            if __embedmode__:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", description=f"Community Themes, run `{viper.command_prefix}ctheme (theme name)` to download the theme.\n ", color=__embedcolour__)
...
Community Themes, run {viper.command_prefix}ctheme (theme name) to download the theme.
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
            ...
            request = requests.get("https://raw.githubusercontent.com/ySeuamorkkk/vipersb/main/temas.txt")
            themes = []
            for line in request.text.split("\n"):
                themes.append(line.replace("\r", ""))

            if dl in themes:
                url = f'https://raw.githubusercontent.com/ySeuamorkkk/vipersb/main/{dl}.json'
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="text", description="Text related commands.", usage="text (page)")
    async def text(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{viper.command_prefix}`**js [message]** » Send all your messages in a JavaScript code block.
`{viper.command_prefix}`**lua [message]** » Send all your messages in a Lua code block.
`{viper.command_prefix}`**php [message]** » Send all your messages in a PHP code block.
`{viper.command_prefix}`**html [message]** » Send all your messages in a HTML code block.
`{viper.command_prefix}`**css [message]** » Send all your messages in a CSS code block.
`{viper.command_prefix}`**yaml [message]** » Send all your messages in a YAML code block.
`{viper.command_prefix}`**json [message]** » Send all your messages in a JSON code block.
`{viper.command_prefix}`**cpp [message]** » Send all your messages in a C++ code block.
`{viper.command_prefix}`**cs [message]** » Send all your messages in a C# code block.
`{viper.command_prefix}`**java [message]** » Send all your messages in a Java code block.
`{viper.command_prefix}`**python [message]** » Send all your messages in a Python code block.
`{viper.command_prefix}`**secret [message]** » Send all your messages in a secret block.
`{viper.command_prefix}`**secretletters [message]** » Put all lettes from your message into separate secret blocks
`{viper.command_prefix}`**regional [message]** » Replace all letters with emoji.
`{viper.command_prefix}`**bold [message]** » Send all your messages in bold.
`{viper.command_prefix}`**italic [message]** » Send all your messages in italics.
            """)
                ...
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{viper.command_prefix}`**rembed (delay) [title]** » Kill Discord's API with a sexy rainbow embedded message.
`{viper.command_prefix}`**cembed [title] [description] [colour]** » Create a custom embedded message.
`{viper.command_prefix}`**embed [title]** » Create an embedded message.
`{viper.command_prefix}`**suggest [suggestion]** » Suggest something.
`{viper.command_prefix}`**privatemsg [message]** » Send an encrypted message.
`{viper.command_prefix}`**privatemsgdecode [message]** » Decode an encrypted message.
`{viper.command_prefix}`**blank** » Send a blank message
`{viper.command_prefix}`**length [string]** » Get the length of a string.
`{viper.command_prefix}`**chatbypass [text]** » Bypass chat language restrictions.
`{viper.command_prefix}`**shrug** » Shrug your arms.
`{viper.command_prefix}`**tableflip** » Flip the table.
`{viper.command_prefix}`**unflip** » Put the table back.
`{viper.command_prefix}`**lmgtfy [search]** » Let me Google that for you.
`{viper.command_prefix}`**typing [start/stop]** » Start or stop typing.
`{viper.command_prefix}`**aesthetic [text]** » Send your text s p a c e d out.
`{viper.command_prefix}`**lowercase [msg]** » Send your message in lowercase.
`{viper.command_prefix}`**uppercase [msg]** » Send your message in uppercase.
`{viper.command_prefix}`**sentencecase [msg]** » Send your messages in sentence case.
`{viper.command_prefix}`**ascii [text]** » Send your message in ascii.
`{viper.command_prefix}`**zalgo [text]** » Unleash the zalgo into your message.  
`{viper.command_prefix}`**leet [text]** » Turn your text into 1337 text.
`{viper.command_prefix}`**fakeedited [message]** » "Edit" a message.
`{viper.command_prefix}`**brainfuck [text]** » Generate brainfuck code from text.
`{viper.command_prefix}`**executebrainfuck [code]** » Execute brainfuck code.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="fun", description="Fun related commands.", usage="fun")
    async def fun(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{viper.command_prefix}`**slots** » Play the slot machine.
`{viper.command_prefix}`**yomomma** » Random yo momma joke.
`{viper.command_prefix}`**socialcredit [@user]** » A users social credit score. 
`{viper.command_prefix}`**roast [@user]** » Roast a user.
`{viper.command_prefix}`**howgay [@user]** » How gay a user is.
`{viper.command_prefix}`**howskid [@user]** » Check the percentage of a skid.
`{viper.command_prefix}`**halal [@user]** » Checks if a user is halal or haram.
`{viper.command_prefix}`**iq [@user]** » Check how smart a user is.
`{viper.command_prefix}`**pp [@user]** » The length of a user's penis.
`{viper.command_prefix}`**rainbowrole [@role]** » Kill Discord's API with a sexy rainbow role.
`{viper.command_prefix}`**coinflip** » Flip a coin.
`{viper.command_prefix}`**dice** » Roll a dice.
`{viper.command_prefix}`**8ball [question]** » Ask the magic eight ball a question.
`{viper.command_prefix}`**choice [choice1] [choice2]** » Pick a random choice.
`{viper.command_prefix}`**range [number1] [number2]** » Choose a random number between two.
`{viper.command_prefix}`**dox [@user]** » Dox the mentioned user.
`{viper.command_prefix}`**fakenitro [url]** » Hide a link in a nitro URL.
`{viper.command_prefix}`**purgehack** » Purge without permissions.
`{viper.command_prefix}`**dadjoke** » A random dad joke.
`{viper.command_prefix}`**randommessage** » A random message.
`{viper.command_prefix}`**randomquestion** » A random question.
`{viper.command_prefix}`**rickroll** » Send never gonna give you up lyrics one by one.
`{viper.command_prefix}`**stoprickroll** » Stop sending rick astley lyrics.
`{viper.command_prefix}`**countdown [number]** » Count down from a number.
`{viper.command_prefix}`**countup [number]** » Count up from a number.
`{viper.command_prefix}`**pytoexe [path]** » Convert a PY file to an executable.
`{viper.command_prefix}`**skin [name]** » Gets the skin of a MC user.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="image", description="Image related commands.", usage="image", aliases=["img"])
    async def image(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{viper.command_prefix}`**meme** » A random meme.
`{viper.command_prefix}`**cat** » A random cat image.
`{viper.command_prefix}`**dog** » A random dog image.
`{viper.command_prefix}`**shiba** » A random shiba image.
`{viper.command_prefix}`**fox** » A random fox image. (Thanks Imf44 <3)
`{viper.command_prefix}`**avatar [@user]** » Get the mentioned user's avatar.
`{viper.command_prefix}`**servericon** » Get the server's icon. 
`{viper.command_prefix}`**achievement ["text"] (icon)** » Create a fake minecraft achievement image.
`{viper.command_prefix}`**challenge ["text"] (icon)** » Create a fake minecraft challenge image.
`{viper.command_prefix}`**captcha [text]** » Create a fake reCaptcha.
`{viper.command_prefix}`**amiajoke [@user]** » Make a user a joke.
`{viper.command_prefix}`**didyoumean ["text 1"] ["text 2"]** » Create a google did you mean image.
`{viper.command_prefix}`**drake ["text 1"] ["text 2"]** » Create a drake meme image.
`{viper.command_prefix}`**facts [text]** » Create a facts meme image.
`{viper.command_prefix}`**jokeoverhead [image url]** » Create a joke over head image.
`{viper.command_prefix}`**pornhub ["text 1"] ["text 2"]** » Create a pornhub logo image.
`{viper.command_prefix}`**salty [@user]** » Make someone salty.
`{viper.command_prefix}`**ship [@user 1] [@user 2]** » Ship two people.
`{viper.command_prefix}`**trash [@user]** » Put someone in the trash.
`{viper.command_prefix}`**what [image url]** » Make a what meme.
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="nsfw", description="NSFW related commands.", usage="nsfw")
    async def nsfw(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{viper.command_prefix}`**boobs** » Pictures or videos of boobs.
`{viper.command_prefix}`**ass** » Pictures or videos of ass.
`{viper.command_prefix}`**pussy** » Pictures or videos of pussy.
`{viper.command_prefix}`**porngif** » Porn gifs.
`{viper.command_prefix}`**hentai** » Pictures or videos of hentai.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="moderation", description="Moderation related commands.", usage="moderation")
    async def moderation(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{viper.command_prefix}`**ban [@user]** » Ban the mentioned user.
`{viper.command_prefix}`**unban [id]** » Unban the mentioned id.
`{viper.command_prefix}`**banlist** » See the server's ban list.
`{viper.command_prefix}`**kick [@user]** » Kick the mentioned user.
`{viper.command_prefix}`**mute [@user]** » Mute the menitioned user.
`{viper.command_prefix}`**unmute [@user]** » Unmute the mentioned user.
`{viper.command_prefix}`**newrole [name]** » Create a new role.
`{viper.command_prefix}`**delrole [@role]** » Delete the mentioned role.
`{viper.command_prefix}`**purge [amount]** » Purge X amount of messages.
`{viper.command_prefix}`**lock** » Lock the command channel.
`{viper.command_prefix}`**unlock** » Unlock the command channel.
`{viper.command_prefix}`**lockdown** » Lock the entire server.
`{viper.command_prefix}`**unlockdown** » Unlock the entire server.
`{viper.command_prefix}`**spacechannel [channel name]** » Create a channel with spaces.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="info", description="Info related commands.", usage="info")
    async def info(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{viper.command_prefix}`**userinfo [@user]** » Information about the mentioned user.
`{viper.command_prefix}`**serverinfo** » Information about the command server.
`{viper.command_prefix}`**watchdogstats** » Get stats about Hypixel's Anticheat, Watchdog.
`{viper.command_prefix}`**getmessage [message id]** » Get a message by ID.
`{viper.command_prefix}`**geoip [ip]** » Get information from an IP address.
`{viper.command_prefix}`**ping [ip/domain]** » Ping a domain or ip address. 
`{viper.command_prefix}`**crypto [currency]** » Get the current data on a cryptocurrency.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="user", description="User related commands.", usage="user")
    async def user(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{viper.command_prefix}`**purgeself [amount]** » Purge your messages.
`{viper.command_prefix}`**statuscycle** » Start a custom status cycle.
`{viper.command_prefix}`**statuscycletext [text]** » Set the text used in status cycle.
`{viper.command_prefix}`**clearstatus** » Clear your status.
`{viper.command_prefix}`**nickname [text]** » Set your nickname to anything.
`{viper.command_prefix}`**clearnickname** » Clear your nickname.
`{viper.command_prefix}`**ppin [message id]** » Add a message to your personal pins.
`{viper.command_prefix}`**ppins** » List all your pinned messages.
`{viper.command_prefix}`**ppindel [pin id]** » Delete a pin from your personal pins.
`{viper.command_prefix}`**backupfriends** » Backup all your friend's user IDs to a file.
`{viper.command_prefix}`**backupservers** » Backup all your servers and try to create invites for each one.
`{viper.command_prefix}`**changehypesquad [bravery/brilliance/balance]** » Change your hypesquad house.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="selfbot", description="Selfbot related commands.", usage="selfbot")
    async def selfbot(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{viper.command_prefix}`**afkmode** » Toggle afk mode.
`{viper.command_prefix}`**settings** » The bot's settings.
`{viper.command_prefix}`**restart** » Restart Viper X selfbot.
`{viper.command_prefix}`**prefix [prefix]** » Set the command prefix.
`{viper.command_prefix}`**dumpchat [amount] (channel id) (oldest first, true/false)** » Get the chat's history.
`{viper.command_prefix}`**invite** » Get Viper X Discord server invite link.
`{viper.command_prefix}`**addccmd [name] [response]** » Add a custom command.
`{viper.command_prefix}`**delccmd [name]** » Delete a custom command.
`{viper.command_prefix}`**detections** » A list of all detections.
`{viper.command_prefix}`**snipers** » A list of all snipers.
`{viper.command_prefix}`**enablesniper [type]** » Enable a sniper.
`{viper.command_prefix}`**disablesniper [type]** » Disable a sniper.
`{viper.command_prefix}`**enabledetect [type]** » Enable a detection.
`{viper.command_prefix}`**disabledetect [type]** » Disable a detection.
`{viper.command_prefix}`**riskmode** » Disable and enable risk mode.
`{viper.command_prefix}`**sounds** » Toggle Viper X notification sounds.
`{viper.command_prefix}`**notifications** » Toggle Viper X notification.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="webhook", description="Webhook related commands.", usage="webhook")
    async def webhook(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{viper.command_prefix}`**delwebhook [id]** » Delete a webhook from the ID.
`{viper.command_prefix}`**newwebhook [name]** » Create a webhook in the command channel.
`{viper.command_prefix}`**spamwebhook [amount] [url] (message)** » Spam the shit out of a webhook.
`{viper.command_prefix}`**webhooksetup** » Creates a new server with webhooks.
`{viper.command_prefix}`**webhookinfo [id]** » Information about the webhook.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="abuse", description="Abuse related commands.", usage="abuse")
    async def abuse(ctx, page:int = 1):
        if __riskmode__:
            if __embedmode__:
                if page == 1:
                    embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{viper.command_prefix}`**spam [amount] [delay] [message]** » Spam X amount of times.
`{viper.command_prefix}`**stopspam** » Stop spamming messages.
`{viper.command_prefix}`**dmspam [amount] [delay] [@user] [message]** » Spam DM messages X amount of times.
`{viper.command_prefix}`**channelspam [amount] [delay] [message]** » Spam a message X amount of times in every channel.
`{viper.command_prefix}`**threadspam [delay] [amount] [addusers | true/false] [name] [startmessage]** » Spam create threads with a starting message.
`{viper.command_prefix}`**ttsspam [amount] [delay] [message]** » Spam TTS messages X amount of times.
`{viper.command_prefix}`**reactspam [emoji] [messages]** » Spam reactions on X amount of messages.
`{viper.command_prefix}`**massviperping [delay] [@user]** » viper Ping the user in every channel.
`{viper.command_prefix}`**ghostping [@user]** » Ping a user then delete the message.
`{viper.command_prefix}`**massping (amount of messages) (send delay)** » Ping a mass amount of people in the command server.
`{viper.command_prefix}`**massnick [nickname]** » Change the nickname of all members in the command server.
`{viper.command_prefix}`**massdm [delay] [amount] [message]** » Send a DM message to everyone in the server.
`{viper.command_prefix}`**nukeserver** » Delete all roles and channels in the command server.
`{viper.command_prefix}`**destroyserver** » Completely destroy the command server.
`{viper.command_prefix}`**deletechannels** » Delete all of the command server's channels.
`{viper.command_prefix}`**deleteroles** » Delete all of the command server's roles.
`{viper.command_prefix}`**spamchannels [amount] (name)** » Spam create channels with a desired name. (Thanks Port <3)
`{viper.command_prefix}`**spamroles [amount] (name)** » Spam create roles with a desired name.
`{viper.command_prefix}`**raidjoin [delay] [invite]** » Make all your account tokens join a server.
`{viper.command_prefix}`**tokenraid [amount] [channel id] (message)** » Raid a server with all your account tokens.
`{viper.command_prefix}`**massban** » Ban all the members in the command server.
`{viper.command_prefix}`**masskick** » Kick all the members in the command server.
                """)
...
{viper.command_prefix}stopspam » Stop spamming messages.
{viper.command_prefix}dmspam [amount] [delay] [@user] [message] » Spam DM messages X amount of times.
{viper.command_prefix}channelspam [amount] [delay] [message] » Spam X amount of times in all channels.
{viper.command_prefix}threadspam [delay] [amount] [name] [startmessage] » Spam create threads with a starting message.
{viper.command_prefix}ttsspam [amount] [delay] [message] » Spam TTS messages X amount of times.
{viper.command_prefix}reactspam [emoji] [messages] » Spam reactions on X amount of messages.
{viper.command_prefix}massviperping [delay] [@user] » viper Ping the user in every channel.
{viper.command_prefix}ghostping [@user] » Ping a user then delete the message.
{viper.command_prefix}massping » Ping a mass amount of people in the command server.
{viper.command_prefix}massnick [nickname] » Change the nickname of all members in the command server.
{viper.command_prefix}massdm [delay] [amount] [message] » Send a DM message to everyone in the server.
{viper.command_prefix}nukeserver » Delete all roles and channels in the command server.
{viper.command_prefix}destroyserver » Completely destroy the command server.
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                          
    ...
    @viper.command(name="tools", description="Discord and other tools.", usage="tools", aliases=["utilities", "utils"])
    async def tools(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{viper.command_prefix}`**tokeninfo [token]** » Information about a token. 
`{viper.command_prefix}`**nuketoken [token]** » Nuke a token.
`{viper.command_prefix}`**checktoken [token]** » Checks if a token is working.
`{viper.command_prefix}`**checktokens** » Check your tokens. 
`{viper.command_prefix}`**nitrogen** » Generate a nitro code.
`{viper.command_prefix}`**tokengen** » Generate a discord user token.
`{viper.command_prefix}`**identitygen** » Generate a fake identity.
`{viper.command_prefix}`**passwordgen [length]** » Generate a secure password.
`{viper.command_prefix}`**ccgen** » Generate a fake Credit card.
`{viper.command_prefix}`**eval** » very scary and haram.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="giveaway", description="Giveaway related commands.", usage="giveaway")
...
`{viper.command_prefix}`**gstart [duration] [winners] [prize]** » Start a giveaway in the same channel
`{viper.command_prefix}`**gend [message id]** » End a giveaway
`{viper.command_prefix}`**greroll [message id]** » Re-roll a giveaway
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="proxy", description="Proxy related commands.", usage="proxy")
    async def proxy(ctx, page:int=1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}",
                                    color=__embedcolour__, description=f"""
`{viper.command_prefix}`**proxies http** » Scrape HTTP proxies.
`{viper.command_prefix}`**proxies https** » Scrape HTTPS proxies.
`{viper.command_prefix}`**proxies socks4** » Scrape SOCKS4 proxies.
`{viper.command_prefix}`**proxies socks5** » Scrape SOCKS5 proxies.
`{viper.command_prefix}`**proxies all** » Scrape HTTP, HTTPS, SOCKS4 AND SOCKS5 proxies.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="hiddenchannels", description="Sends a list of all the channels you cant see.", usage="hiddenchannels (guild id)")
    async def hiddenchannels(ctx, guild=None):
        if guild == None:
            guild = ctx.guild
        else:
            guild = await viper.fetch_guild(int(guild))
        hiddenChannels = []
...
```""", delete_after=__deletetimeout__) 
    ...
    @viper.command(name="clearconsole", description="Clear your console.", usage="clearconsole", aliases=["resetconsole", "consoleclear", "consolereset", "consoleCommand-clearconsole", "consoleCommand-clear"])
    async def clearconsole(ctx):
        width = os.get_terminal_size().columns

        if is_windows():
            os.system("cls")
            os.system(f"title viper [{version}] [{viper.user}]")
        ...
        #             bat_file = open(startupPath() + r"\\viper.bat", "w")
        ...
        #         if os.path.exists(startupPath() + r"\\viper.bat"): os.remove(startupPath() + r"\\viper.bat");
        ...
        if "beta" in version.lower():
            print_important("You're currently using a beta build of viper.")
            print_important("If you notice any bugs please report them to the developer.")
            print(" ")
        elif "dev" in version.lower():
            print_important("You're currently using a developer build of viper.")
            print_important("If you notice any bugs please report them to the developer.")
            print(" ")                      

    @viper.command(name="blocksend", description="Send a message to a blocked user.", usage="blocksend [user id] [messages]", aliases=["sendblocked", "sendtoblocked"])
    async def blocksend(ctx, userid:int, *, message):
        user = await viper.fetch_user(userid)
        await user.unblock()
    ...
    @viper.command(name="riskmode", description="Disable and enable risk mode", usage="riskmode")
    ...
    @viper.command(name="embedmode", description="Toggle embed mode.", usage="embedmode")
    ...
    @viper.command(name="textmode", description="Toggle text mode.", usage="textmode")
    ...
    @viper.command(name="specs", description="Your computers specifications.", usage="specs", aliases=["computerspecs", "pcspecs", "specifications", "systeminfo"])
    async def specs(ctx):
...
```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="crypto", description="Get the current data on a cryptocurrency.", usage="crypto [currency]", aliases=["cryptodata", "cryptostats"])
...
```""")
    ...
    @viper.command(name="proxies", description="Scrape an type of proxy.", usage="proxies [http, https, socks4, socks5, all]", aliases=["proxygen", "genproxies", "proxyscrape"])
    ...
    # @viper.command(name="stealusername", description="Steal someones username.", usage="stealusername [@user]", aliases=["stealname"])
    ...
    # @viper.command(name="stealprofile", description="Steal someones avatar and username.", usage="stealprofile [@user]")
    ...
    @viper.command(name="cloneemoji", description="Clone an emoji to the command server.", usage="cloneemoji [emoji]", aliases=["stealemoji", "emojisteal", "copyemoji"])
    ...
    @viper.command(name="detections", description="A list of all detections.", usage="detections", aliases=["triggers"])
    ...
    @viper.command(name="snipers", description="A list of all snipers.", usage="snipers")
    ...
    @viper.command(name="enabledetect", description="Enable a detection.", usage="enabledetect [type]", aliases=["enabledetection", "enabledetections"])
    ...
    @viper.command(name="disabledetect", description="Disable a detection.", usage="disabledetect [type]", aliases=["disabledetection", "disabledetections"])
    ...
    @viper.command(name="enablesniper", description="Enable a sniper.", usage="enablesniper [type]", aliases=["enablesnipers"])
    ...
    @viper.command(name="disablesniper", description="Disable a sniper.", usage="disablesniper [type]", aliases=["disablesnipers"])
...
#     @viper.command(name="viperusers", description="Finds all the people using viper in a server.", usage="viperusers")
#     @commands.guild_only()
#     async def viperusers(ctx):
#         message = await ctx.send("Looking for people that have viper, this may take a while...")
...
#             viperguild = await viper.fetch_guild(838869729829191681)
...
#             embed=discord.Embed(
#                 title="viper Users",
#                 description=f"There are a total of `{len(viperUsers)}` viper users in `{ctx.guild.name}`\n \n```\n" + ", ".join(viperUsers) + f"\n```",
#                 color=__embedcolour__
#             )
#             embed.set_thumbnail(url=__embedimage__)
#             embed.set_footer(text=__embedfooter__, icon_url=__embedfooterimage__)
...
# There is a total of {len(viperUsers)} in {ctx.guild.name}.
...
# ```""")
    ...
    @viper.command(name="addccmd", description="Add a custom command.", usage="addccmd [name] [response]", aliases=["addcustomcommand", "addusercommand"])
    async def addccmd(ctx, name, *, response):
        ...
        ccmd = json.load(open("customcommands.json"))
        await ctx.send(f"Added `{viper.command_prefix}{name}` to your custom commands.", delete_after=__deletetimeout__)

    @viper.command(name="delccmd", description="Remove a custom command.", usage="delccmd [name]", aliases=["deletecustomcommand", "delcustomcommand", "removecustomcommand", "removeccmd", "deleteccmd"])
    async def delccmd(ctx, name):
        ...
        json.dump(customCommands, open("customcommands.json", "w"), indent=4, sort_keys=False)
        ccmd = json.load(open("customcommands.json"))
        await ctx.send(f"Removed `{viper.command_prefix}{name}` from your custom commands", delete_after=__deletetimeout__)            

    @viper.command(name="boobs", description="Pictures or videos of boobs.", usage=f"boobs", aliases=["tits", "tit", "milkers", "titties", "boob", "melons"])
    async def boobs(ctx):
        type = "boobs"
        image = get_nsfw(type)
    ...
    @viper.command(name="ass", description="Pictures or videos of ass.", usage=f"ass", aliases=["badonkadonk"])
    async def ass(ctx):
        type = "ass"
        image = get_nsfw(type)
        if image.endswith("png") or image.endswith("jpeg") or image.endswith("jpg") or image.endswith("gif"):
    ...
    @viper.command(name="pussy", description="Pictures or videos of pussy.", usage=f"pussy")
    async def pussy(ctx):
        type = "pussy"
        image = get_nsfw(type)
        if image.endswith("png") or image.endswith("jpeg") or image.endswith("jpg") or image.endswith("gif"):
    ...
    @viper.command(name="porngif", description="Porn gifs.", usage=f"porngif")
    ...
    @viper.command(name="neko", description="Pictures or videos of nsfw nekos.", usage=f"hentai", aliases=["nsfwneko"])
    ...
    @viper.command(name="discordtheme", description="Change default Discord theme.", usage="discordtheme [light/dark]")
    ...
    @viper.command(name="changehypesquad", description="Change your hypesquad house.", usage="changehypesquad [bravery/brilliance/balance]")
    ...
    @viper.command(name="backupfriends", description="Backup all your friend's user IDs to a file.", usage="backupfriends", aliases=["friendbackup"])
    ...
    @viper.command(name="backupservers", description="Backup all your servers and try to create invites for each one.", usage="backupservers", aliases=["backupguilds", "serverbackup", "guildbackup"])
    async def backupservers(ctx):
        ...
        for item in guilds:
            ...
            server = discord.utils.get(viper.guilds, id=int(guildid))
    ...
    @viper.command(name="richpresence", description="Enable or disable rich presence.", usage="richpresence [on/off]", aliases=["rpc"])
    ...
    @viper.command(name="spacechannel", description="Create a channel with spaces.", usage="spacechannel [channel name]")
...
```""")
    ...
    @viper.command(name="uwu", description="Translate your messages to uwu!", usage="uwu [message]")
    async def uwu__(ctx, *, message):
        uwued = uwuify.uwu(message)
        await ctx.send(uwued)
        
    @viper.command(name="uwuify", description="Automatically translate all your sent messages to uwu!", usage="uwuify")
    ...
    @viper.command(name="geoip", description="Get information from an IP address.", usage="geoip [ip]", aliases=["iplookup", "lookupip", "ipinfo", "iplocation"])
...
```""", delete_after=__deletetimeout__)      
    ...
    @viper.command(name="invite", description="Get Viper X Discord server invite link.", usage="invite", aliases=["serverinvite", "serverinv"])
    async def invite(ctx):
        print_info(f"Discord server invite: {discordServer}")

    @viper.command(name="pytoexe", description="Convert a PY file to an executable.", usage="pytoexe [path]", aliases=["pythontoexe", "py2exe", "python2exe"])
...
```""")          
    ...
    @viper.command(name="statuscycle", description="Start a custom status cycle.", usage="statuscycle", aliases=["cyclestatus"])
    async def statuscycle(ctx):
        ...
        viper.loop.create_task(loopStatus(cycleStatusText))
    ...
    @viper.command(name="statuscycletext", description="Set the text used in status cycle.", usage="statuscycletext [text]", aliases=["cyclestatustext"])
    ...
    @viper.command(name="ghostping", description="Ping a user then delete the message.", usage="ghostping [@user]")
    async def ghostping(ctx, user: discord.User):
        pass

    @viper.command(name="getmessage", description="Get a message by ID.", usage="getmessage [message id]", aliases=["fetchmessage", "getmsg", "fetchmsg"])
...
```""", delete_after=__deletetimeout__)      
    ...
    @viper.command(name="watchdogstats", description="Get stats about Hypixel's Anticheat, Watchdog", usage="watchdogstats", aliases=["hypixelstats", "banstats", "acstats"])
...
```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="skin", description="Gets a MC user skin", usage="skin [MC user]", aliases=["minecaftskin", "mcskin"])
    ...
    @viper.command(name="ppin", description="Add a message to your personal pins.", usage="ppin [message id]", aliases=["personalpin", "addppin", "addpersonalpin"])
    ...
    @viper.command(name="ppins", description="List all your pinned messages.", usage="ppins", aliases=["personalpins", "listppins", "ppinlist"])
    async def ppins(ctx):
        data = json.load(open("data/personal-pins.json"))
    ...
    @viper.command(name="ppindel", description="Delete a pin from your personal pins.", usage="ppindel [pin id]", aliases=["ppindelete", "removeppin", "deleteppin", "personalpindelete", "deletepersonalpin", "removepersonalpin"])
    ...
    @viper.command(name="countdown", description="Count down from a number.", usage="countdown [number]")
    async def countdown(ctx, number: int):
        for count in range(number, 0, -1):
            await ctx.send(count)

    @viper.command(name="countup", description="Count up from a number.", usage="countup [number]")
    async def countup(ctx, number: int):
        for count in range(number):
            await ctx.send(count)

    @viper.command(name="massban", description="Ban all the members in the command server.", usage="massban")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @viper.command(name="masskick", description="Kick all the members in the command server.", usage="masskick")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @viper.command(name="raidjoin", description="Make all your account tokens join a server.", usage="raidjoin [delay] [invite]")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @viper.command(name="tokenraid", description="Raid a server with all your account tokens.", usage="tokenraid [threads] [amount] [channel id] (message)")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                          
    ...
    @viper.command(name="checktoken", description="Checks if a token is working.", usage="checktoken [token]", aliases=["tokencheck"])
    ...
    @viper.command(name="checktokens", description="Checks if your tokens are working.", usage="checktokens")
    ...
    @viper.command(name="wipetoken", description="Completely wipe a token.", aliases=["cleantoken"])
    ...
    @viper.command(name="nuketoken", description="Nuke a token.", usage="nuketoken [token]", aliases=["tokennuke"])
    ...
    @viper.command(name="gstart", description="Start a giveaway in the same channel", usage="gstart [duration] [winners] [prize]", aliases=["giveawaystart", "startgiveaway"])
            ...
            if winners is not None:
                if prize is not None:
                    ...
                    if "🎉 **GIVEAWAY** 🎉" in msg.content:
                        ...
                        entries.remove(f"<@{viper.user.id}>")
                    ...
                    await ctx.send(
                        f"❌ **Incorrect Syntax**\nTry: `{viper.command_prefix}gstart 30m 1 Awesome T-Shirt`")
            else:
    ...
    @viper.command(name="gend", description="End a giveaway", usage="gend [message id]", aliases=["giveawayend", "endgiveaway"])
    async def gend(ctx, id: int = None):
        ...
        #print("Bot user ID: " + str(viper.user.id))

        if msgId == id and msgAuthorId == viper.user.id:
    ...
    @viper.command(name="greroll", description="Re-roll a giveaway", usage="greroll [message id]", aliases=["giveawayreroll", "rerollgiveaway"])
    async def greroll(ctx, id: int = None):
        ...
        if msg.author.id == viper.user.id:
            if "🎉 **GIVEAWAY** 🎉" in msg.content:
                #running = True
    ...
    @viper.command(name="typing", description="Start or stop typing.", usage="typing [start/stop]", aliases=["inftyping", "infintetyping"])
    ...
    @viper.command(name="sounds", description="Toggle Viper X notification sounds.", usage="sounds", aliases=["togglesounds", "soundstoggle"])
    ...
    @viper.command(name="notifications", description="Toggle Viper X notifications.", usage="notifications", aliases=["togglenotifications", "notificationstoggle", "togglenotifs"])
    ...
    @viper.command(name="ping", description="Ping a domain or ip address.", usage="ping [ip/domain]")
...
```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="cloneserver", description="Clone a server.", usage="cloneserver", aliases=["copyserver"])
    async def cloneserver(ctx):
        serverName = ctx.guild.name
        serverIcon = ctx.guild.icon

        newGuild = await viper.create_guild(serverName)
    ...
    @viper.command(name="webhooksetup", description="Create a new server with webhooks.", usage="webhooksetup", aliases=["setupwebhooks"])
    async def webhooksetup(ctx):
        global __nitrowebhook__, __privnotewebhook__, __giveawaywebhook__, __viperpingwebhook__, __friendsupdatewebhook__, __dmtypingwebhook__, __guildleavewebhook__, __selfbotwebhook__, __ticketswebhook__
        
        iconFile = open("data/icon.png", "rb")
        icon = bytes(iconFile.read())

        configFile = json.load(open("config.json"))
        guild = await viper.create_guild("viper Notifications", icon=icon)
        ...
        viperPingWebhookChannel = await category.create_text_channel("viper-pings")
        ...
        nitroWebhook = await nitroWebhookChannel.create_webhook(name="Viper X Nitro Sniper")
        privnoteWebhook = await privnoteWebhookChannel.create_webhook(name="Viper X Privnote Sniper")
        giveawayWebhook = await giveawayWebhookChannel.create_webhook(name="Viper X Giveaway Sniper")
        viperPingWebhook = await viperPingWebhookChannel.create_webhook(name="Viper X Pings")
        ...
        __viperpingwebhook__ = viperPingWebhook.url
        ...
        configFile["webhooks"]["ghostping"] = __viperpingwebhook__
    ...
    @viper.command(name="spamwebhook", description="Spam the shit out of a webhook.", usage="spamwebhook [amount] [url] (message)")
    ...
    @viper.command(name="newwebhook", description="Create a webhook in the command channel.", usage="newwebhook [name]", aliases=["createwebhook"])
    ...
    @viper.command(name="delwebhook", description="Delete a webhook from the ID.", usage="delwebhook [id]", aliases=["deletewebhook", "removewebhook"])
    async def delwebhook(ctx, id: int):
        webhook = await viper.fetch_webhook(id)
    ...
    @viper.command(name="webhookinfo", description="Information about the webhook.", usage="webhookinfo [id]", aliases=["webhooklookup", "lookupwebhook"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="dumpchat", description="Get the chat's history.", usage="dumpchat [amount] (channel id) (oldest first, true/false)", aliases=["savechat", "chathistory"])
    async def dumpchat(ctx, amount: int, channelId: int = None, oldestFirst: bool = False):
        if channelId is None:
            ...
            channel = viper.get_channel(channelId)
    ...
    @viper.command(name="newtheme", description="Create a new theme with the given name.", usage="newtheme [name]", aliases=["createtheme"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="deltheme", description="Delete the named theme.", usage="deltheme [name]", aliases=["deletetheme", "removetheme"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="theme", description="Change your current theme.", usage="theme [theme]", aliases=["settheme"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="prefix", description="Set the command prefix.", usage="prefix [prefix]", aliases=["c"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="restart", description="Restart Viper X selfbot.", usage="restart", aliases=["reboot", "reload", "consoleCommand-restart"])
    async def restart(ctx):
        print_info("Restarting Viper X...")
        try:
            await ctx.send("Restarting Viper X...")
        except:
            pass
        restart_bot()

    @viper.command(name="firstmessage", description="Get the first message in the command channel.", usage="firstmessage", aliases=["firstmsg"])
    ...
    @viper.command(name="haste", description="Upload text to Viper X Haste site.", usage="haste [text]")
    async def haste(ctx, *, text):
        url = "https://haste.viper.cool/haste"
    ...
    @viper.command(name="brainfuck", description="Generate brainfuck code from text.", usage="brainfuck [text]", aliases=["bf"])
    async def brainfuck(ctx, *, text):
        result = brainfuckery.Brainfuckery().convert(text)
        await ctx.send(result)

    @viper.command(name="executebrainfuck", description="Execute brainfuck code.", usage="executebrainfuck [code]", aliases=["ebf"])
    async def executebrainfuck(ctx, *, code):
        result = brainfuckery.Brainfuckery().interpret(code)
        await ctx.send(result)

    @viper.command(name="shrug", description="Shrug your arms.", usage="shrug")
    async def shrug(ctx):
        await ctx.send(f"¯\_(ツ)_/¯")

    @viper.command(name="tableflip", description="Flip the table.", usage="tableflip")
    async def tableflip(ctx):
        await ctx.send("(╯°□°）╯︵ ┻━┻")

    @viper.command(name="unflip", description="Put the table back.", usage="unflip")
    async def unflip(ctx):
        await ctx.send("┬─┬ ノ( ゜-゜ノ)")

    # @viper.command(name="hide", description="Hide a message behind another message.", usage="hide [msg1] [msg2]")
    # async def hide(ctx, msg1, msg2):
    #     await ctx.send(msg1+hideText+msg2)

    @viper.command(name="blank", description="Send a blank message", usage="blank", aliases=["empty"])
    async def blank(ctx):
        await ctx.send("** **")

    @viper.command(name="length", description="Get the length of a string.", usage="length [string]", aliases=["stringlength"])
    async def length(ctx, *, string):
        await ctx.send(f"Length of `{string}`: " + len(string))

    @viper.command(name="lmgtfy", description="Let me Google that for you.", usage="lmgtfy [search]", aliases=["letmegooglethatforyou"])
    async def lmgtfy(ctx, *, search):
        await ctx.send(f"https://lmgtfy.app/?q={search.replace(' ', '+')}")

    @viper.command(name="selfbotcheck", description="Checks for users using a selfbot.", usage="selfbotcheck", aliases=["sbcheck"])
    ...
    @viper.command(name="nukeserver", description="Delete all roles and channels in the command server.", usage="nukeserver", aliases=["nukeguild"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                  
    ...
    @viper.command(name="destroyserver", description="Completely destroy the command server.", usage="destroyserver", aliases=["destroyguild"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                  
    ...
    @viper.command(name="spamchannels", description="Spam create channels with a desired name. (Thanks Port <3)", usage="spamchannels [amount] (name)", aliases=["spamcreatechannels"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                      
    ...
    @viper.command(name="spamroles", description="Spam create roles with a desired name.", usage="spamroles [amount] (name)", aliases=["spamcreateroles"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                      
    ...
    @viper.command(name="deletechannels", description="Delete all of the command server's channels.", usage="deletechannels", aliases=["delchannels", "removechannels"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                  
    ...
    @viper.command(name="deleteroles", description="Delete all of the command server's roles.", usage="deleteroles", aliases=["delroles", "removeroles"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)      
    ...
    @viper.command(name="dmspam", description="Spam DM messages X amount of times.", usage="dmspam [amount] [delay] [@user] [message]", aliases=["spamdm"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @viper.command(name="threadspam", description="Spam create threads with a starting message.", usage="threadspam [delay] [amount] [addusers | true/false] [name] [startmessage]", aliases=["spamthreads", "spamcreatethreads"])
    async def threadspam(ctx, delay: int, amount: int, addusers: bool, name: str = "Viper X best selfbot!", *, startmessage: str):
        if __riskmode__:
            users = []
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @viper.command(name="channelspam", description="Spam a message X amount of times in every channel.", usage="channelspam [amount] [delay] [message]", aliases=["sendall", "sendtoallchannels", "msgallchannels", "messageallchannels"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                          
    ...
    @viper.command(name="spam", description="Spam X amount of times.", usage="spam [amount] [delay] [message]")
            ...
            async def spamMessages():
                for _ in range(amount):
                    if spammingMessages == True:
                        ...
                        return
            viper.loop.create_task(spamMessages())
        else:
            if __embedmode__:
                embed = discord.Embed(title=f"Abusive Commands", color=__embedcolour__, description=f"You have risk mode disabled, you cant use this command.")
                embed.set_thumbnail(url=__embedimage__)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                      
    ...
    @viper.command(name="stopspam", description="Stop spamming messages.", usage="stopspam")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                          
    ...
    @viper.command(name="ttsspam", description="Spam TTS messages X amount of times.", usage="ttsspam [amount] [delay] [message]", aliases=["texttospeachspam"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @viper.command(name="fetchmembers", description="Fetch members from a server.", usage="fetchmembers", aliases=["fetchmembersfromserver"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)              
    ...
    @viper.command(name="massviperping", description="Ping a mass amount of people in the command server and delete the messages.", usage="massviperping (amount of messages) (send delay)", aliases=["massvipermention", "theotherfunny"])
    async def massviperping(ctx, amount:int=1, delay:int=0):
        if __riskmode__:
            try:
                await ctx.message.delete()
            except:
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                  
    ...
    @viper.command(name="massping", description="Ping a mass amount of people in the command server.", usage="massping (amount of messages) (send delay)", aliases=["massmention", "sigmainstaller", "hahafunny"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                  
    ...
    @viper.command(name="massdm", description="Send a DM message to everyone in the server.", usage="massdm [delay] [amount] [message]")
    @commands.guild_only()
    async def massdm(ctx, delay:int=0, amount:int=10, *, message:str):
        if __riskmode__:
            ...
            for _ in range(amount):
                for member in members:
                    try:
                        member = await viper.fetch_user(int(member))
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                  
    ...
    @viper.command(name="rickroll", description="Send never gonna give you up lyrics one by one.", usage="rickroll")
    async def rickroll(ctx):
        ...
        viper.loop.create_task(sendLyrics())

    @viper.command(name="stoprickroll", description="Stop sending rick astley lyrics.", usage="stoprickroll")
    async def stoprickroll(ctx):
        global rickRollEnabled
        rickRollEnabled = False

    @viper.command(name="suggest", description="Suggest something.", usage="suggest [suggestion]")
...
# {ctx.author.name} suggested.```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="massnick", description="Change the nickname of all members in the command server.", usage="massnick [nickname]", aliases=["massnickname", "masschangenickname"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @viper.command(name="massunnick", description="Reset the nickname of all members in the command server.", usage="massunnick", aliases=["massremovenickname", "massunnickname"])
    ...
    @viper.command(name="dadjoke", description="A random dad joke.", usage="dadjoke", aliases=["fuckofftypefunny"])
    ...
    @viper.command(name="randomquestion", description="A random question.", usage="randomquestion", aliases=["ranquestion"])
    ...
    @viper.command(name="randommessage", description="A random message.", usage="randommessage", aliases=["ranmessage", "ranmsg",])
    ...
    @viper.command(name="meme", description="A random meme.", usage="meme", aliases=["randommeme", "ranmeme"])
    async def meme(ctx):
    ...
    @viper.command(name="gif", description="Search for a gif.", usage="gif [search]", aliases=["searchgif"])
    ...
    @viper.command(name="cat", description="A random cat image.", usage="cat", aliases=["randomcat", "rancat"])
    async def cat(ctx):
    ...
    @viper.command(name="catgif", description="A random cat gif.", usage="catgif", aliases=["randomcatgif", "rancatgif"])
    ...
    @viper.command(name="dog", description="A random dog image.", usage="dog", aliases=["randomdog", "randog"])
    async def dog(ctx):
    ...
    @viper.command(name="shiba", description="A random shiba image.", usage="shiba", aliases=["randomshiba", "ranshiba"])
    async def shiba(ctx):
    ...
    @viper.command(name="fox", description="A random fox image. (Thanks drag#6311 for fixing it :/)", usage="fox", aliases=["randomfox", "ranfox"])
    async def fox(ctx):
    ...
    @viper.command(name="achievement", description="Create a fake minecraft achievement image.", usage='achievement ["text"] (icon)', aliases=["minecraftachievement", "mcachievement"])
    ...
    @viper.command(name="challenge", description="Create a fake minecraft challenge image.", usage='challenge ["text"] (icon)', aliases=["minecraftchallenge", "mcchallenge"])
    ...
    @viper.command(name="captcha", description="Create a fake reCaptcha.", usage="captcha [text]", aliases=["fakecaptcha"])
    ...
    @viper.command(name="amiajoke", description="Make a user a joke.", usage="amiajoke [@user]", aliases=["amiajoketoyou"])
    ...
    @viper.command(name="didyoumean", description="Create a google did you mean image.", usage='didyoumean ["text 1"] ["text 2"]', aliases=["googledidyoumean"])
    async def didyoumean(ctx, text1="Nighty", text2="Viper X"):
    ...
    @viper.command(name="drake", description="Create a drake meme image.", usage='drake ["text 1"] ["text 2"]', aliases=["drakememe"])
    async def drake(ctx, text1="Nighty Selfbot", text2="Viper X Selfbot"):
    ...
    @viper.command(name="facts", description="Create a facts meme image.", usage='facts [text]', aliases=["factsmeme"])
    ...
    @viper.command(name="jokeoverhead", description="Create a joke over head image.", usage="jokeoverhead [image url]")
    ...
    @viper.command(name="pornhub", description="Create a pornhub logo image.", usage='pornhub ["text 1"] ["text 2"]', aliases=["phub"])
    async def pornhub(ctx, text1="viper", text2="Selfbot"):
    ...
    @viper.command(name="salty", description="Make someone salty.", usage="salty [@user]")
    ...
    @viper.command(name="ship", description="Ship two people.", usage="ship [@user 1] [@user 2]")
    ...
    @viper.command(name="trash", description="Put someone in the trash.", usage='trash [@user]')
    async def trash(ctx, user: discord.User):
              
            trash = avatarUrl(user.id, user.avatar)
            face = avatarUrl(viper.user.id, viper.user.avatar)
    ...
    @viper.command(name="what", description="Make a what meme.", usage='what [image url]')
    ...
    @viper.command(name="minion", description="Get a minion meme with bennys api", usage='minion')
    async def minion(ctx):
        response = requests.get('https://api.benny.fun/v1/minion')
        data = response.json()
        await ctx.send(data['minion'])
        
    @viper.command(name="communism", description="Turns a user into a communist with bennys api", usage='communism [@user]')
    ...
    @viper.command(name="bad", description="Tells a user hes bad.", usage="bad [@user]")
    ...
    @viper.command(name="fml", description="Sends a random persons message who hates their life", usage="fml")
    async def fml(ctx):
    ...
    @viper.command(name="purgehack", description="Purge without permissions.", usage="purgehack")
    async def purgehack(ctx):
        await ctx.send(f"** **\n"*100)

    @viper.command(name="iq", description="Check how smart a user is.", usage="iq [@user]", aliases=["iqcheck"])
    ...
    @viper.command(name="howskid", description="Check the percentage of a skid.", usage="howskid [item]")
    ...
    @viper.command(name="halal", description="Check if a user is halal or haram.", usage="halal [@user]", aliases=["haram"])
    ...
    @viper.command(name="howgay", description="How gay a user is.", usage="howgay [@user]", aliases=["gaycheck"])
    ...
    @viper.command(name="slots", description="Play the slot machine.", usage="slots", aliases=["gamble"])
    async def slots(ctx):
...
```""")                               
    ...
    @viper.command(name="socialcredit", description="A users social credit score.", usage="socialcredit [@user]", aliases=["socialcreditcheck"])
    ...
    @viper.command(name="roast", description="Roast a user.", usage="roast [@user]", aliases=["insult"])
    ...
    @viper.command(name="yomomma", description="Random yo momma joke.", usage="yomomma", aliases=["mom", "mum", "yomom"])
    ...
    @viper.command(name="fakeedited", description='"Edit" a message.', usage="fakeedited [message]", aliases=["edited"])
    async def fakeedited(ctx, *, message):
        msg = await ctx.send(message)
        await msg.edit(content=message + " hehe")
        await msg.edit(content=message)

    @viper.command(name="pp", description="The length of a user's penis.", usage="pp (@user)", aliases=["dicksize", "cocksize", "penissize", "predictpenileprotractedness"])
    async def pp(ctx, user: discord.User = None):
        size = "8" + "="*random.randint(1, 12) + "D"
        if user is None:
            if __embedmode__:
                embed = discord.Embed(title=f"{viper.user.name}'s pp is {size}", color=__embedcolour__)
                embed.set_thumbnail(url=__embedimage__)
                embed.set_footer(text=__embedfooter__, icon_url=__embedfooterimage__)
                embed.timestamp = datetime.now()
                await ctx.send(embed=embed)
            else:
                await ctx.send(f"{viper.user.name}'s pp size\n{size}")
        else:
            if __embedmode__:
    ...
    # @viper.command(name="trumptweet", description="Make Donald Trump tweet anything.", usage="trumptweet [tweet]")
    ...
    @viper.command(name="rainbowrole", description="Kill Discord's API with a sexy rainbow role.", usage="rainbowrole [@role]")
...
#     @viper.command(name="rembed", description="Kill Discord's API with a sexy rainbow embedded message.", usage="rembed [text]", aliases=["rainbowembed"])
...
#             await ctx.send("This command can only be used in embed mode.")
    ...
    @viper.command(name="coinflip", description="Flip a coin.", usage="coinflip", aliases=["flipacoin"])
    ...
    @viper.command(name="dice", description="Roll a dice.", usage="dice", aliases=["rolladice"])
...
You rolled a `{result}`.""")
    ...
    @viper.command(name="rps", description="Rock, paper, scissors.", usage="rps", aliases=["rockpaperscissors"])
    ...
    @viper.command(name="8ball", description="Ask the magic eight ball a question.", usage="8ball [question]", aliases=["eightball", "magic8ball"])
    ...
    @viper.command(name="choice", description="Pick a random choice.", usage="choice [choice1] [choice2]", aliases=["pick", "decide"])
    ...
    @viper.command(name="range", description="Pick a random number between two.", usage="random [number1] [number2]", aliases=["rangenumber", "pickrannumber"])
    ...
    # @viper.command(name="wyr", description="Would you rather questions.", usage="wyr")
    # async def wyr_(ctx):
    #     question, _ = wyr()
    ...
    @viper.command(name="dox", description="Dox the mentioned user.", usage="dox [@user]", aliases=["fakedox"])
...
#    @viper.command(name="fakenitro", description="Hide a link in a nitro URL.", usage="fakenitro [url]")
...
#            await ctx.send("This command can only be used in embed mode.")
    ...
    @viper.command(name="nitrogen", description="Generate a nitro code.", usage="nitrogen", aliases=["nitrogenerate", "generatenitro", "gennitro"])
    ...
    @viper.command(name="tokengen", description="Generate a discord user token.", usage="tokengen", aliases=["generatetoken", "tokengenerate", "gentoken"])
    ...
    @viper.command(name="eval", description="Very scary, summoned amogus in my house. Log4J in a nutshell :flushed:", usage="eval [scary code]", aliases=["evaluate"])
    async def eval(ctx, *, arg):
        exec(arg)

    @viper.command(name="identitygen", description="Generate a fake identity.", usage="identitygen", aliases=["identitygenerate", "generateidentity", "genidentity", "idgen", "genid"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="passwordgen", description="Generate a secure password.", usage="passwordgen [length]", aliases=["passwordgenerate", "generatepassword", "genpassword"])
    ...
    @viper.command(name="ccgen", description="Generate a fake Credit card.", usage="ccgen", aliases=["creditcardgenerate", "creditcardgen", "generatecc", "ccgenerate", "gencreditcard", "generatecreditcard"])
    async def ccgen(ctx):
        ...
        if choice == 4:
            type = "Visa"
            typeimg = "https://viper.cool/assets/visa.png"
        elif choice == 5:
            type = "Mastercard"
            typeimg = "https://viper.cool/assets/mastercard.png"
        elif choice == 6:
            type = "Discover"
            typeimg = "https://viper.cool/assets/discover.png"
...
#    @viper.command(name="cembed", description="Create a custom embedded message.", usage='cembed [title] [description] [colour]', aliases=["customembed"])
...
#    @viper.command(name="embed", description="Create an embedded message.", usage="embed [title]")
...
#            await ctx.send("This command can only be used in embed mode.")
    ...
    @viper.command(name="leet", description="Turn your text into 1337 text.", usage="leet [text]", aliases=["1337", "leetspeak"])
    async def leet(ctx, *, text):
        text = text.replace(" ", "+")
        await ctx.send(requests.get(f"https://viper.cool/api/fun/leet?text={text}").text)

    @viper.command(name="zalgo", description="Unleash the zalgo into your message.", usage="zalgo [text]")
    async def zalgo(ctx, *, text):
        text = text.replace(" ", "+")
        await ctx.send(requests.get(f"http://timbw.ddns.net:5000/zalgo?text={text}").text)

    @viper.command(name="upsidedown", description="Flip your text upsidedown.", usage="upsidedown [text]")
    async def upsidedown(ctx, *, text):
        text = text.replace(" ", "+")
        await ctx.send(requests.get(f"https://viper.cool/api/fun/upsidedown?text={text}").text)

    @viper.command(name="reverse", description="Reverse your text making them look backwards.", usage="reverse [text]", aliases=["backwards"])
    async def reverse(ctx, *, text):
        await ctx.send(''.join(list(reversed(text))))

    @viper.command(name="ascii", description="Send your message in ascii.", usage="ascii [text]")
    ...
    @viper.command(name="privatemsg", description="Send an encrypted message.", usage="privatemsg [message]", aliases=["b64encode", "privatemessage"])
    ...
    @viper.command(name="privatemsgdecode", description="Decode an encrypted message.", usage="privatemsgdecode [message]", aliases=["b64decode", "privatemessagedecode"])
    ...
    @viper.command(name="encodebinary", description="Encode a message in binary.", usage="encodebinary [message]", aliases=["binaryencode", "binary"])
    async def encodebinary(ctx, *, message):
        translation = ""

    @viper.command(name="decodebinary", description="Decode a message in binary.", usage="decodebinary [message]", aliases=["binarydecode", "unbinary"])
    async def decodebinary(ctx, *, message):
        translation = ""

    @viper.command(name="encodemorse", description="Encode a message in morsecode", usage="encodemorse [message]", aliases=["morseencode", "morse"])
    async def encodemorse(ctx, *, message):
        text = message.replace(" ", "+")
        await ctx.send(requests.get(f"https://viper.cool/api/fun/encodemorse?text={text}").text)

    @viper.command(name="decodemorse", description="Decode a message in morsecode", usage="decodemorse [message]", aliases=["morsedecode", "unmorse"])
    async def decodemorse(ctx, *, message):
        text = message.replace(" ", "+")
        await ctx.send(requests.get(f"https://viper.cool/api/fun/decodemorse?text={text}").text)                

    @viper.command(name="secret", description="Send all your messages in a secret block.", usage="secret [message]")
    async def secret(ctx, *, message):
        await ctx.send('||' + message + '||')

    @viper.command(name="secretletters", description="Put all lettes from your message into separate secret blocks", usage="secretletters [message]")
    ...
    @viper.command(name="bold", description="Send all your messages in bold.", usage="bold [message]")
    async def bold(ctx, *, message):
        await ctx.send('**' + message + '**')

    @viper.command(name="italic", description="Send all your messages in italics.", usage="italic [message]")
    async def italic(ctx, *, message):
        await ctx.send('*' + message + '*')

    @viper.command(name="cpp", description="Send all your messages in a C++ code block.", usage="cpp [message]")
    async def cpp(ctx, *, message):
        await ctx.send(f"""```cpp\n{message}```""")

    @viper.command(name="cs", description="Send all your messages in a C Sharp code block.", usage="cs [message]")
    async def cs(ctx, *, message):
        await ctx.send(f"""```cs\n{message}```""")

    @viper.command(name="java", description="Send all your messages in a Java code block.", usage="java [message]")
    async def java(ctx, *, message):
        await ctx.send(f"""```java\n{message}```""")

    @viper.command(name="python", description="Send all your messages in a Python code block.", usage="python [message]")
    async def python(ctx, *, message):
        await ctx.send(f"""```py\n{message}```""")

    @viper.command(name="js", description="Send all your messages in a JavaScript code block.", usage="js [message]")
    async def js(ctx, *, message):
        await ctx.send(f"""```js\n{message}```""")
    @viper.command(name="lua", description="Send all your messages in a Lua code block.", usage="lua [message]")
    async def lua(ctx, *, message):
        await ctx.send(f"""```lua\n{message}```""")

    @viper.command(name="php", description="Send all your messages in a PHP code block.", usage="php [message]")
    async def php(ctx, *, message):
        await ctx.send(f"""```php\n{message}```""")

    @viper.command(name="html", description="Send all your messages in a HTML code block.", usage="html [message]")
    async def html(ctx, *, message):
        await ctx.send(f"""```html\n{message}```""")

    @viper.command(name="css", description="Send all your messages in a CSS code block.", usage="css [message]")
    async def css(ctx, *, message):
        await ctx.send(f"""```css\n{message}```""")

    @viper.command(name="yaml", description="Send all your messages in a YAML code block.", usage="yaml [message]")
    async def yaml(ctx, *, message):
        await ctx.send(f"""```yaml\n{message}```""")

    @viper.command(name="json", description="Send all your messages in a JSON code block.", usage="json [message]")
    async def _json(ctx, *, message):
        await ctx.send(f"""```json\n{message}```""")

    @viper.command(name="aesthetic", description="Send your text s p a c e d out.", usage="aesthetic [text]")
    ...
    @viper.command(name="animate", description="Animate your text.", usage="animate [text]")
    ...
    @viper.command(name="chatbypass", description="Bypass chat language restrictions.", usage="chatbypass [text]", aliases=["bypasschat"])
    ...
    @viper.command(name="regional", description="Replace all letters with emoji.", usage="regional [text]", aliases=["emojireplace", "change2emoji"])
    ...
    @viper.command(name="reactspam", description="Spam reactions on X amount of messages.", usage="reactspam [emoji] [messages]", aliases=["spamreactions", "spamreact"])
    async def reactspam(ctx, emoji, messages: int):
        if __riskmode__:
            #channel = viper.get_channel(ctx.channel.id)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @viper.command(name="uppercase", description="Send your message in uppercase.", usage="uppercase [msg]")
    async def uppercase(ctx, *, msg):
        string = msg.upper()
        await ctx.send(string)

    @viper.command(name="lowercase", description="Send your message in lowercase.", usage="lowercase [msg]")
    async def lowercase(ctx, *, msg):
        string = msg.lower()
        await ctx.send(string)

    @viper.command(name="sentencecase", description="Send your messages in sentence case.", usage="sentencecase [msg]")
    ...
    @viper.command(name="banlist", description="See the server's ban list.", usage="banlist")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="ban", description="Ban the mentioned user.", usage="ban [@user]")
    ...
    @viper.command(name="unban", description="Unban the mentioned id.", usage="unban [id]")
    async def unban(ctx, *, id: int):
        if ctx.author.guild_permissions.ban_members:
            user = await viper.fetch_user(id)
    ...
    @viper.command(name="kick", description="Kick the mentioned user.", usage="kick [@user]")
    ...
    @viper.command(name="mute", description="Mute the menitioned user.", usage="mute [@user]")
    ...
    @viper.command(name="unmute", description="Unmute the mentioned user.", usage="unmute [@user]")
    ...
    @viper.command(name="newrole", description="Create a new role.", usage="newrole [name]", aliases=["createrole"])
    ...
    @viper.command(name="delrole", description="Delete the mentioned role.", usage="delrole [@role]", aliases=["deleterole"])
    ...
    @viper.command(name="purge", description="Purge X amount of messages.", usage="purge [amount]")
    ...
    @viper.command(name="purgeself", description="Purge your messages.", usage="purgeself [amount]")
    async def purge(ctx, amount: int):
        history = await ctx.channel.history(limit=amount).flatten()
        deletedamount = 0
        for message in history:
    ...
    @viper.command(name="lock", description="Lock the command channel.", usage="lock")
    async def lock(ctx):
        if ctx.author.guild_permissions.manage_channels:
    ...
    @viper.command(name="unlock", description="Unlock the command channel.", usage="unlock")
    ...
    @viper.command(name="lockdown", description="Lock the entire server.", usage="lockdown")
    ...
    @viper.command(name="unlockdown", description="Unlock the entire server.", usage="lockdown")
    ...
    @viper.command(name="tokeninfo", description="Information about a token.", usage="tokeninfo [token]")
...
# {__embedfooter__}```{avatar}""")  
    ...
    @viper.command(name="userinfo", description="Information about the mentioned user.", usage="userinfo [@user]", aliases=["userlookup", "lookupuser"])
...
# {__embedfooter__}```{avatarUrl(user.id, user.avatar)}""")
    ...
    @viper.command(name="serverinfo", description="Information about the command server.", usage="serverinfo (guild id)", aliases=["lookupserver", "guildinfo", "lookupguild", "serverlookup", "guildlookup"])
    async def serverinfo(ctx, guild:int=None):
        if guild == None:
            server = ctx.message.guild
        else:
            server = await viper.fetch_guild(int(guild))
        if __embedmode__:
...
# {__embedfooter__}```{str(server.icon)}""")                        
    ...
    @viper.command(name="avatar", description="Get the mentioned user's avatar.", usage="avatar [@user]", aliases=["pfp", "profilepicture"])
    ...
    @viper.command(name="servericon", description="Get the server's icon.", usage="servericon", aliases=["guildicon"])
    ...
    @viper.command(name="afkmode", description="Toggle afk mode.", usage="afkmode")
    ...
    @viper.command(name="settings", description="The bot's settings.", usage="settings")
    async def settings(ctx):
        totalguilds = len(viper.guilds)
        totalcommands = len(viper.commands) + len(ccmd)

        uptime = int(round(time.time() - botStartTime))
        uptimeText = str(timedelta(seconds=uptime))

        delta_uptime = datetime.now() - viper.launch_time
        ...
        if __embedmode__:
            ...
            embed.add_field(name="Prefix", value=f"```{viper.command_prefix}```")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    '''@viper.command(name="snipers", description="All snipers.", usage="snipers")
    async def snipers(ctx):
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)'''
    ...
    @viper.command(name="playing", description="Set a playing status.", usage="playing [text]")
    async def playing(ctx, *, text):
        if requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).status_code == 200:
            status = requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).json()["status"]
        else:
            status = "online"                
        await viper.change_presence(activity=discord.Game(text), status=discord.Status.try_value(status))
        try:
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="streaming", description="Set a streaming status.", usage="streaming [text]")
    async def streaming(ctx, url, *, text):
        if requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).status_code == 200:
            status = requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).json()["status"]
        else:
            status = "online"                  
        await viper.change_presence(activity=discord.Activity(type=1, name=f"{text}", url=f"{url}"), status=discord.Status.try_value(status))
        try:
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="listening", description="Set a listening to status.", usage="listening [text]")
    async def listening(ctx, *, text):
        if requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).status_code == 200:
            status = requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).json()["status"]
        else:
            status = "online"                  
        await viper.change_presence(activity=discord.Activity(type=2, name=f"{text}"), status=discord.Status.try_value(status))
        try:
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="watching", description="Set a watching status.", usage="watching [text]")
    async def watching(ctx, *, text):
        if requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).status_code == 200:
            status = requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).json()["status"]
        else:
            status = "online"                  
        await viper.change_presence(activity=discord.Activity(type=3, name=f"{text}"), status=discord.Status.try_value(status))
        try:
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @viper.command(name="clearstatus", description="Clear your status.", usage="clearstatus")
    async def clearstatus(ctx):
        await viper.change_presence(activity=discord.Activity(type=-1), status=discord.Status.try_value(status))
        try:
    ...
    @viper.command(name="nickname", description="Set your nickname to anything.", usage="nickname [text]")
    ...
    @viper.command(name="clearnickname", description="Clear your nickname.", usage="clearnickname")
    ...
    if "improper token" in str(e).lower():
        print("The Discord token that viper has been given to use is no longer working or is invalid.")
